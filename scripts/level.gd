...
func _ready():
	...
	for child in waypoints:
		waypointPositions.append(child.global_position)
		print_debug(waypointPositions)
...
# Health Bar
func setup_healthbar(player: Character):
	health_bar.show()
	player.health_changed.connect(_on_player_health_changed)
	if player.health_changed.is_connected(_on_player_health_changed):
		print("Signal erfolgreich verbunden für Spieler: ", player.name)
	...
	if new_health <= 1:
		if players_container.has_node(str(owner_id)):
			...
			for mob in mobs_container.get_children():
				if mob.is_class("CharacterBody3D"):
					var player_data = mob.body_map[owner_id]
