...
const DASH_INVINCIBILITY = 0.4
const DASH_COOLDOWN = 0.5
@onready var head = $"3DGodotRobot/RobotArmature"
@onready var camera = $SpringArmOffset/SpringArm3D
@onready var body = $MeshInstance3D
@onready var body_collider = $CollisionShape3D
@onready var hitbox = $Head/Hitbox
@onready var idleTimer = $"State Machine/IdleTimer"
var canAttack = true
var can_dash = true
var HEALTH = 100
...
@rpc func take_damage(DAMAGE, hasKnockback) -> void:
	if hasKnockback == true:
		pass
	var new_life = HEALTH - DAMAGE
	HEALTH = new_life
	print_debug("Playerhealth: ", HEALTH)
